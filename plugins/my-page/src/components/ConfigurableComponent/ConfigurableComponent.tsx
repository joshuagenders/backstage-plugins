...
export const ConfigurableComponent = 
    ({ id, entityRef }: {id: string, entityRef?: string }) => {
    const { componentFactory } = useContext(ComponentFactoryContext)
    const catalogApi = useApi(catalogApiRef)
    const { value: entity, loading, error } = useAsync(async () => {
        if (!entityRef) return undefined
        const [kind, namespace, name] = entityRef.split(':').map((e: string) => e.split('/')).flat()
    ...
    if (entity){
        return <EntityProvider entity={entity}>
            {componentFactory(id, {})}
        </EntityProvider>
    }
